...
import stryker4jvm.config.{Full, MutationScoreOnly}
import stryker4jvm.reporting.FinishedRunEvent
import stryker4jvm.reporting.dashboard.DashboardConfigProvider
import stryker4jvm.reporting.model.{DashboardConfig, DashboardPutResult}
import stryker4jvm.reporting.reporters.DashboardReporter
...
class DashboardReporterTest extends Stryker4jvmIOSuite with MockitoIOSuite with LogMatchers {
  ...
  def baseResults = {
    ...
    FinishedRunEvent(report, metrics, 15.seconds, Path("target/stryker4s-report/").toNioPath)
  }
