...
public abstract class Logger {
  ...
  /**
   * Creates a new logger. Whether color is enabled or not depends on {@link
  ...
  /**
   * Creates a new logger with the provided colorEnabled value and thus ignores {@link
   * #determineColorEnabled()}.
   *
  ...
  /**
   * Function that determines whether color is enabled on the terminal used. It does so on a
   * best-effort basis. Classes that extend from {@link #Logger} may override this function.
   * However, the programmer must make sure that this test does not depend on any field of the
   * subclass as these may not be initialised yet when this is invoked.
   *
  ...
  protected boolean determineColorEnabled() {
    ...
    boolean windowsEnabled =
        System.getenv().containsKey("WT_SESSION")
            || System.getenv().containsKey("ConEmuANSI")
                && System.getenv("ConEmuANSI").equals("ON");
